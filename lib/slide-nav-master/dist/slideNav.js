...
var SlideNav = function () {
	...
	_createClass(SlideNav, [{
		...
		value: function setActiveAnchor() {
			...
			try {
				for (var _iterator2 = this.navAnchors[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
					...
					if (section && (section.offsetTop - 216 <= offset && section.offsetTop - 216 + section.offsetHeight > offset || offset + window.innerHeight == scrollHeight)) {
		...
		value: function goToSection(linkHash) {
			...
			let navChkHeight = $(window).height() * 0.1194029850746269;
			console.log(navChkHeight)
			if (section) {
				var offsetTop = section.offsetTop - navChkHeight;
